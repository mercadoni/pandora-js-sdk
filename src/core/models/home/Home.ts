  ...
  static fromJson(json: Record<string, any>): Home {
    const widgets = Array.isArray(json.widgets)
      ? json.widgets.map((widgetData: any) => {
          if (
            widgetData.dataSource === "CAROUSEL" &&
            Array.isArray(widgetData.data?.items)
          ) {
            widgetData.data.carousels = widgetData.data.items.map(
              (item: Record<string, any>) => Carrusel.fromJson(item),
            );
          } else if (
            widgetData.dataSource === "CATEGORY_AND_PRODUCTS" &&
            Array.isArray(widgetData.data?.categories)
          ) {
            widgetData.data.categories = widgetData.data.categories.map(
              (item: Record<string, any>) => Category.fromJson(item),
            );
          } else if (
            widgetData.dataSource === "FEATURED_CATEGORIES" &&
            Array.isArray(widgetData.data?.categories)
          ) {
            widgetData.data.categories = widgetData.data.categories.map(
              (item: Record<string, any>) => Category.fromJson(item),
            ...
            widgetData.data.products = widgetData.data.items.map(
              (item: Record<string, any>) => Product.fromJson(item),
            );
          } else if (
    ...
    if (
      isValid(json.id) &&
      isValid(json.breakpoint) &&
      isValid(json.targetType) &&
      Array.isArray(json.storeReferences)
    ) {
      return new Home({
      ...
      throw new Error(
        "Invalid home json: id, targetType, and storeReferences array are required",
      );
    }
  }
}
